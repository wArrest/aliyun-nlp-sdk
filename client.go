...
func (n NlpClient) send(method string, urlStr string, body []byte) (*http.Response, error) {
  req, _ := http.NewRequest(method, urlStr, bytes.NewReader(body))

  headerMap := n.initSignature(req, body)
...
func (n NlpClient) initSignature(req *http.Request, body []byte) headerMap {
